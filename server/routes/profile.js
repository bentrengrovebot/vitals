...
router.get('/', async (req, res) => {
  try {
    ...
    if (req.baseUrl === '/api/goals') {
      ...
      return res.json(goals || { calories: 2300, proteinG: 150, fatG: 80, carbsG: 250, waterMl: 2500 });
    }
...
router.put('/', async (req, res) => {
  try {
    if (req.baseUrl === '/api/goals') {
      const { calories, proteinG, fatG, carbsG, waterMl } = req.body;
      const goals = await req.prisma.goals.create({
        data: { userId: req.userId, calories, proteinG, fatG, carbsG, waterMl: waterMl || 2500 },
      });
