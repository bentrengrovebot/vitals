...
const inp = { width: '100%', padding: '14px 16px', borderRadius: 12, border: '1px solid rgba(255,255,255,0.1)', background: '#232a33', fontSize: 15, boxSizing: 'border-box', color: '#e2e4e9' };
...
export default function More({ goTo, onRefresh, openRecipeEdit }) {
  ...
  const menuItem = (label, subtitle, onClick) => (
    <button onClick={onClick} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', padding: '18px 18px', background: 'none', border: 'none', borderTop: '1px solid #252a31', color: t1 }}>
  ...
  if (!section) return (
    <div style={{ paddingBottom: 92 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '20px 20px 8px' }}>
        ...
        <button onClick={logout} style={{ padding: '7px 14px', borderRadius: 10, border: '1px solid rgba(255,255,255,0.08)', background: '#252a31', color: t2, fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 1 }}>Logout</button>
      </div>
      ...
      <div style={{ padding: '0 16px' }}>
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 6 }}>
                {['Male', 'Female'].map(s => (
                  <button key={s} onClick={() => setProfile(p => ({ ...p, sex: s }))}
                    style={{ padding: '8px 20px', borderRadius: 12, border: profile.sex === s ? '1px solid #2dba8e' : '1px solid rgba(255,255,255,0.08)', background: profile.sex === s ? 'rgba(45,186,142,0.12)' : '#252a31', color: profile.sex === s ? ac : t2, fontSize: 13, fontWeight: 600 }}>{s}</button>
                ))}
              </div>
            </div>
        ...
        {section === 'recipes' && (
          <div>
            ...
            {recipes.map(r => {
              ...
              return (
                <div key={r.id} onClick={() => openRecipeEdit(r)} style={{ background: '#252a31', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, padding: '16px 18px', marginBottom: 8, cursor: 'pointer' }}>
        ...
        {section === 'supps' && (
          <div>
            {supplements.filter(s => s.isActive).map(sup => (
              <div key={sup.id} style={{ background: '#252a31', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, padding: '14px 16px', marginBottom: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
            ...
            {addingSupp ? (
              <div style={{ background: '#252a31', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, padding: 18, marginTop: 8 }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <button onClick={() => { setAddingSupp(false); setSuppForm({ name: '', ingredient: '', dose: '' }); }} style={{ flex: 1, padding: 12, borderRadius: 12, border: '1px solid rgba(255,255,255,0.08)', background: '#252a31', color: t1, fontSize: 14, fontWeight: 600 }}>Cancel</button>
        ...
        {section === 'weight' && (
          <div>
            ...
            {weighIns.map(w => (
              <div key={w.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 0', borderBottom: '1px solid #252a31' }}>
                <div>
        ...
        {section === 'data' && (
          <div>
            <div style={{ background: '#252a31', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, padding: 18, marginBottom: 16 }}>
            ...
            {!confirmClear ? (
              ...
              <div style={{ background: '#252a31', border: '2px solid #f85149', borderRadius: 14, padding: 18 }}>
                ...
                <div style={{ display: 'flex', gap: 10 }}>
                  <button onClick={() => setConfirmClear(false)} style={{ flex: 1, padding: 12, borderRadius: 12, border: '1px solid rgba(255,255,255,0.08)', background: '#252a31', color: t1, fontSize: 14, fontWeight: 600 }}>Cancel</button>
