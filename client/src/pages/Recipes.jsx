  ...
  return (
    <div style={{ paddingBottom: 100 }}>
      <div style={{ display: 'flex', alignItems: 'center', padding: 'calc(env(safe-area-inset-top, 16px) + 8px) 20px 20px', gap: 12 }}>
        <div style={{ fontSize: 20, fontWeight: 800, flex: 1, color: '#212121' }}>Recipes</div>
        ...
        {recipes.map(r => {
          ...
          return (
            <div key={r.id} onClick={() => onEdit(r)} style={{ ...card, padding: '16px 18px', marginBottom: 10, cursor: 'pointer' }}>
              <div style={{ fontSize: 15, fontWeight: 700, color: '#212121' }}>{r.name || 'Untitled'}</div>
              <div style={{ fontSize: 12, color: t2, marginTop: 4 }}>{r.servings} serve{r.servings !== 1 ? 's' : ''} · {ps.cal} cal · {ps.protein}g P</div>
            </div>
          );
        })}
        {!recipes.length && <div style={{ color: '#757575', fontSize: 14, padding: 30, textAlign: 'center' }}>No recipes yet.</div>}
      </div>
    </div>
  );
}
