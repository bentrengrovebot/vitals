...
export default function Settings({ goTo, onRefresh }) {
  ...
  const ac = '#3b82f6', gn = '#22c55e', or = '#f59e0b', rd = '#ef4444', t1 = '#111827', t2 = '#6b7280', t3 = '#9ca3af', brd = '#eaeaef', pu = '#8b5cf6';

  const pill = (active) => ({
    padding: '7px 16px', borderRadius: 20, fontSize: 13, fontWeight: 600,
    border: active ? 'none' : '1.5px solid #eaeaef',
    background: active ? ac : '#fff', color: active ? '#fff' : t2,
  });
    ...
    return (
      <div style={{ ...card, padding: 18, marginBottom: 16 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
          ...
          {wkCh !== null && <div style={{ fontSize: 13, fontWeight: 700, padding: '4px 10px', borderRadius: 8, background: wkCh < 0 ? gn + '15' : wkCh > 0 ? rd + '15' : brd, color: wkCh < 0 ? gn : wkCh > 0 ? rd : t2 }}>{wkCh > 0 ? '+' : ''}{wkCh} kg/wk</div>}
        </div>
        <svg width="100%" viewBox={`0 0 ${W} ${H}`} style={{ overflow: 'visible' }}>
          ...
          {pts.map((p, i) => <circle key={i} cx={p.x} cy={p.y} r="4" fill={brd} stroke="#fff" strokeWidth="2" />)}
          <path d={trendLine} fill="none" stroke={ac} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
          {pts.map((p, i) => <circle key={`t${i}`} cx={p.x} cy={p.ty} r="4" fill={ac} stroke="#fff" strokeWidth="2" />)}
          {profile.weightGoalKg && (() => { const gy = pad + (1 - (profile.weightGoalKg - mn) / rng) * (H - pad * 2); if (gy > 0 && gy < H) return (<><line x1={pad} y1={gy} x2={W - pad} y2={gy} stroke={gn} strokeWidth="1.5" strokeDasharray="6,4" /><text x={W - pad + 4} y={gy + 4} fontSize="9" fill={gn}>Goal</text></>); return null; })()}
  ...
  return (
    <div style={{ paddingBottom: 100, background: '#fff', minHeight: '100vh' }}>
      <div style={{ display: 'flex', alignItems: 'center', padding: 20, gap: 12 }}>
        <div style={{ fontSize: 20, fontWeight: 800, flex: 1 }}>Settings</div>
      ...
      {saved && <div style={{ margin: '0 20px 12px', padding: '10px 16px', borderRadius: 12, background: gn + '10', color: gn, fontSize: 14, fontWeight: 600 }}>{saved}</div>}
      ...
      <div style={{ padding: '0 20px' }}>
          ...
          <div>
            {[{ k: 'name', l: 'Name', t: 'text', p: 'Ben' }, { k: 'dob', l: 'Date of Birth', t: 'date' }, { k: 'heightCm', l: 'Height (cm)', t: 'number', p: '176' }, { k: 'weightKg', l: 'Current Weight (kg)', t: 'number', p: '102.2' }, { k: 'weightGoalKg', l: 'Weight Goal (kg)', t: 'number', p: '95' }].map(f => (
              <div key={f.k} style={{ marginBottom: 14 }}>
                <label style={{ fontSize: 11, fontWeight: 700, color: t3, textTransform: 'uppercase' }}>{f.l}</label>
            ...
            <div style={{ marginBottom: 14 }}>
              <label style={{ fontSize: 11, fontWeight: 700, color: t3, textTransform: 'uppercase' }}>Sex</label>
        ...
        {tab === 'goals' && (
          <div>
            ...
            <div style={{ marginBottom: 14 }}>
              <label style={{ fontSize: 11, fontWeight: 700, color: t3, textTransform: 'uppercase' }}>Daily Water Goal (mL)</label>
        ...
        {tab === 'weight' && (
          <div>
            <div style={{ ...card, padding: 18, marginBottom: 16 }}>
              <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 10 }}>Log Weigh-in</div>
              <div style={{ display: 'flex', gap: 8 }}>
            ...
            {weighIns.map(w => (
              <div key={w.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 0', borderBottom: `1px solid ${brd}` }}>
                <div>
                  <div style={{ fontSize: 15, fontWeight: 600 }}>{w.weightKg} kg</div>
                  <div style={{ fontSize: 12, color: t3 }}>{fmtDate(w.date?.split('T')[0] || '')}</div>
                </div>
        ...
        {tab === 'data' && (
          <div>
            <div style={{ ...card, padding: 18, marginBottom: 16 }}>
              <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 4 }}>Your Data</div>
              <div style={{ fontSize: 13, color: t2, lineHeight: 1.6, marginBottom: 16 }}>
                {weighIns.length} weigh-ins · {supplements.length} supplements
              </div>
            </div>
            {!confirmClear ? (
              <button onClick={() => setConfirmClear(true)} style={{ width: '100%', padding: 14, borderRadius: 12, border: `1px solid ${rd}30`, background: rd + '08', color: rd, fontSize: 14, fontWeight: 600 }}>Clear All Data</button>
            ) : (
